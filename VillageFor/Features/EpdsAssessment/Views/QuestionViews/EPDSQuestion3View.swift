...
struct EPDSQuestion3View: View {
    
    @StateObject private var viewModel: EPDSQuestion3ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion3ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion4View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion3View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

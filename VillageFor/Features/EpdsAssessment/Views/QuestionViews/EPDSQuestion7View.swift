...
struct EPDSQuestion7View: View {
    
    @StateObject private var viewModel: EPDSQuestion7ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion7ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion8View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion7View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

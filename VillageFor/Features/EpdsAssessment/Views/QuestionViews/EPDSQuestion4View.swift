...
struct EPDSQuestion4View: View {
    
    @StateObject private var viewModel: EPDSQuestion4ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion4ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion5View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion4View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

...
struct EPDSQuestion5View: View {
    
    @StateObject private var viewModel: EPDSQuestion5ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion5ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion6View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion5View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

...
struct EPDSQuestion10View: View {
    
    @StateObject private var viewModel: EPDSQuestion10ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion10ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            // final question, now we will navigate to the checklist.
            EPDSChecklistView(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion10View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

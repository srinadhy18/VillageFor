...
struct EPDSQuestion6View: View {
    
    @StateObject private var viewModel: EPDSQuestion6ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion6ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion7View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion6View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

...
struct EPDSQuestion2View: View {
    
    @StateObject private var viewModel: EPDSQuestion2ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion2ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion3View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion2View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}

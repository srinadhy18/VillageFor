...
struct EPDSQuestion8View: View {
    
    @StateObject private var viewModel: EPDSQuestion8ViewModel
    ...
    init(assessment: EPDSAssessment) {
        _viewModel = StateObject(wrappedValue: EPDSQuestion8ViewModel(assessment: assessment))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $viewModel.shouldNavigateToNext) {
            EPDSQuestion9View(assessment: viewModel.assessment)
...
#Preview {
    NavigationStack {
        ...
        EPDSQuestion8View(assessment: sampleAssessment)
            .environmentObject(SessionManager())
    }
}
